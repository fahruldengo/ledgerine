...
const NAV = [
  ...
  ['invoice-list.html','Invoice','M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M9 13h6M9 17h6'],
  ['receipt-list.html','Kwitansi','M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1zM8 8h8M8 12h8'],
...
// halaman editor dipetakan ke item menu daftar-nya, agar highlight menu tetap aktif
// saat membuka form input (invoices.html/receipts.html) dari dalam daftar.
const NAV_ALIAS = {
  'invoices.html':'invoice-list.html',
  'receipts.html':'receipt-list.html'
};

function buildLayout(active){
  const u = Auth.guard(); if(!u) return null;
  const activeNav = NAV_ALIAS[active] || active; // form editor → highlight menu daftar-nya
  ...
  const navHtml = NAV.filter(n=>!n[3]||n[3]===u.role).map(n=>{
    ...
    const act = href===activeNav?'active':'';
    ...
    <div class="main">
      <div class="topbar">
        <div class="flex items-center gap">
          ...
          <strong>${esc(NAV.find(n=>n[0]===activeNav)?.[1]||'Ledgerine')}</strong>
        </div>
